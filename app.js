...
const CACHE_KEY = "NammaDealsCache";
...
async function fetchAmazonData(url) {
  ...
  try {
    ...
    const title =
      (text.match(/"productTitle"[^>]*>(.*?)<\/span>/i) || [])[1]
        ?.replace(/<\/?[^>]+(>|$)/g, "")
        ?.trim() || "Amazon Product";
    ...
    const data = { title, price_text: price, image: img };
...
async function loadLinks() {
  ...
  for (const item of json.items) {
    const link = item.url.includes("?tag=")
      ? item.url
      : item.url + (item.url.includes("?") ? "&" : "?") + "tag=" + AFFILIATE_TAG;
    const details = await fetchAmazonData(link);
    results.push({
      title: details.title,
      image: details.image,
  ...
  for (const p of itemsToShow) {
    grid.innerHTML += `
      <div class="item-card">
        ...
        <a class="nd-button" href="${p.url}" target="_blank">View Deal</a>
      </div>
    `;
  }
}
